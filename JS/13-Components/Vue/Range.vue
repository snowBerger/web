<template>
	<div ref="wrap" :class="['vue-slider-wrap', flowDirection, disabledClass, { 'vue-slider-has-label': piecewiseLabel }]" v-show="show" :style="wrapStyles" @click="wrapClick">
		<div ref="elem" aria-hidden="true" class="vue-slider" :style="[elemStyles, bgStyle]">
            ...
            <span class="dot-click-area" @touchstart="moveStart(0)"></span>
            ...
            <span class="dot-click-area" @touchstart="moveStart(1)"></span>
				...
				<template v-else>
					...
					>
            <span class="dot-click-area" @touchstart="moveStart"></span>
			...
			<template v-else>
				<template v-if="isRange">
					...
					>
            <span class="dot-click-area" @mousedown="moveStart(0)"></span>
						<span :class="['vue-slider-tooltip-' + tooltipDirection[0], 'vue-slider-tooltip-wrap']">
							<slot name="tooltip" :value="val[0]" :index="0">
								<span class="vue-slider-tooltip" :style="tooltipStyles[0]">{{ formatter ? formatting(val[0]) : val[0] }}</span>
							</slot>
					...
					>
            <span class="dot-click-area" @mousedown="moveStart(1)"></span>
						<span :class="['vue-slider-tooltip-' + tooltipDirection[1], 'vue-slider-tooltip-wrap']">
							<slot name="tooltip" :value="val[1]" :index="1">
								<span class="vue-slider-tooltip" :style="tooltipStyles[1]">{{ formatter ? formatting(val[1]) : val[1] }}</span>
							</slot>
				...
				<template v-else>
					<div
						...
						@mousedown="moveStart"
					>
            <span class="dot-click-area" @mousedown="moveStart"></span>
						<span :class="['vue-slider-tooltip-' + tooltipDirection, 'vue-slider-tooltip-wrap']">
...
<script>
  export default {
    ...
    methods: {
      bindEvents () {
        if (this.isMobile) {
          this.$refs.wrap.addEventListener('touchmove', this.moving, { passive: false })
          this.$refs.wrap.addEventListener('touchend', this.moveEnd, { passive: false })
        } else {
          ...
          window.addEventListener('resize', this.refresh, { passive: false })
        }
      },
      unbindEvents () {
        window.removeEventListener('resize', this.refresh, { passive: false })
        if (this.isMobile) {
          this.$refs.wrap.removeEventListener('touchmove', this.moving, { passive: false })
          this.$refs.wrap.removeEventListener('touchend', this.moveEnd, { passive: false })
        } else {
